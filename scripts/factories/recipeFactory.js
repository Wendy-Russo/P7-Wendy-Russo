...
function refreshTags(tagsArray,dom,color){
...
function refreshRecipes(){
    ...
    for (let recipesID = 0; recipesID < recipesFound.length; recipesID++) {
        recipesGallery.appendChild(recipeFactory(recipesFound[recipesID]).getRecipeDom());
    }
}

function refreshIngredients(){

    removeChildren(ingredientsDropdown);

    for (let ingredientsID = 0; ingredientsID < ingredientsFound.length; ingredientsID++) {
        ingredientsDropdown.appendChild(recipeFactory(ingredientsFound[ingredientsID]).getDropdownOptionDom());
    }
}

function refreshAppliances(){

    removeChildren(appliancesDropdown);

    for (let appliancesID = 0; appliancesID < appliancesFound.length; appliancesID++) {
        appliancesDropdown.appendChild(recipeFactory(appliancesFound[appliancesID]).getDropdownOptionDom());
    }
}

function refreshUstensils(){

    removeChildren(ustensilsDropdown);

    for (let ustensilsID = 0; ustensilsID < ustensilsFound.length; ustensilsID++) {
        ustensilsDropdown.appendChild(recipeFactory(ustensilsFound[ustensilsID]).getDropdownOptionDom());
    }
}